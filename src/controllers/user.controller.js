class UserController {
  async createUser(req, res) {
    ...
    try {
      const res = await pool.query(query, [login, name, role]);
      return res.rows[0];
    } catch (err) {
      console.error(err);
      throw err;
  ...
  async updateUser(req, res) {
    const {login, name, avatarUrl} = req.body;
    const query = `UPDATE User SET login = $1, name = $2, avatarUrl = $3 WHERE login = $1 RETURNING *`;

    try {
      const res = await pool.query(query, [login, name, avatarUrl]);
      return res.rows[0];
    } catch (err) {
      console.error(err);
      throw err;
